...
namespace MantisLODEditorOnline {
	class Mantis_Mesh {
		public Mesh mesh;
		public string uuid;
	...
	class MantisLODEditorOnline: Editor {
		...
		public override void OnInspectorGUI() {
			if(target) {
				if(Mantis_Meshes != null) {
					show_title = EditorGUILayout.Foldout(show_title, "Mantis LOD Editor - Online Version 2.1");
					if(show_title) {
		...
		private void get_all_meshes() {
			Component[] allFilters = (Component[])((GameObject)target).GetComponentsInChildren (typeof(MeshFilter));
			Component[] allRenderers = (Component[])((GameObject)target).GetComponentsInChildren (typeof(SkinnedMeshRenderer));
